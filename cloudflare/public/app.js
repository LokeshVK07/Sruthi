...
function playbackCandidatesForSong(song) {
  if (!song) return [];
  const ordered = state.hostedDirectAudio
    ? [song.audioUrl, song.audio128Url, song.audio320Url, song.localAudio320Url, song.localAudio128Url]
    : [song.audioUrl, song.localAudio320Url, song.localAudio128Url, song.audio128Url, song.audio320Url];
...
async function loadCollectionView() {
  if (state.currentView === "playlist" && currentPlaylist()?.official) {
    const playlist = currentPlaylist();
    try {
      const payload = await fetchJson(`/api/playlist?id=${encodeURIComponent(playlist.id)}`);
      ...
      // fall back to cached hydration below
    }
  }
...
async function loadAppState() {
  ...
  state.hostedDirectAudio = Boolean(payload.features?.hostedDirectAudio);
